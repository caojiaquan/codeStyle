...
　　　　[4.1.1 元素获取](#411-%E5%85%83%E7%B4%A0%E8%8E%B7%E5%8F%96)

　　　　[4.1.2 样式获取](#412-%E6%A0%B7%E5%BC%8F%E8%8E%B7%E5%8F%96)

　　　　[4.1.3 样式设置](#413-%E6%A0%B7%E5%BC%8F%E8%AE%BE%E7%BD%AE)

　　　　[4.1.4 DOM 操作](#414-dom-%E6%93%8D%E4%BD%9C)

　　　　[4.1.5 DOM 事件](#415-dom-%E4%BA%8B%E4%BB%B6)
...
##### [建议] 对于 `if...else...`、`try...catch...finally` 等语句，下列关键字后：else, catch, finally，不需要换行
...
##### [建议] 在 `if / else / for / do / while` 语句中，即使只有一行，也不要省略块 `{...}`。
...
##### [建议] `Promise对象` 用 `动宾短语的进行时` 表达，以大写P结尾。

示例：

```javascript
var loadingDataP = ajax.get('url');
loadingDataP.then(callback);
```

##### [强制] `jquery对象`必须以`$`开头命名。
...
##### [强制] 双斜线后，必须跟一个空格；缩进与下一行代码保持一致；可位于一个代码行的末尾，与代码间隔一个空格。
...
##### [建议] 为了便于代码阅读和自文档化，以下内容须包含以 `/**...*/` 形式的块注释中。参考[JSDoc Guide](http://yuri4ever.github.io/jsdoc/)
...
##### [建议] 对 Object 中各项的描述， 使用 `@param` 标识。
